...
const Profile = () => {
  ...
  const learningPreferences = {
    ...
    preferredCategories: user?.preferences?.selectedCategories || ['Web Development', 'AI/ML', 'DevOps'],
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600 mb-1">Preferred Difficulty</p>
                <p className="text-lg font-semibold text-gray-900">{learningPreferences.preferredDifficulty}</p>
              </div>

              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600 mb-1">Learning Style</p>
                <p className="text-lg font-semibold text-gray-900">{learningPreferences.learningStyle}</p>
              </div>

              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600 mb-1">Daily Goal</p>
                <p className="text-lg font-semibold text-gray-900">{learningPreferences.dailyGoal}</p>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6 text-center">
            <div className="w-24 h-24 bg-primary rounded-full flex items-center justify-center text-4xl text-white mx-auto mb-4">
              {user?.name?.charAt(0)?.toUpperCase() || 'U'}
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-1">{user?.name || 'User'}</h3>
            <p className="text-gray-600 text-sm mb-4">{user?.email || 'user@example.com'}</p>
              ...
              {badges.map((badge) => (
                ...
                >
                  ...
                  <p className={`text-xs text-center font-medium ${
                    badge.earned ? 'text-primary-700' : 'text-gray-600'
                  }`}>
                    {badge.name}
                  </p>
                </div>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              <button className="w-full py-2 px-4 text-left text-gray-700 hover:bg-gray-100 rounded-lg transition-all">
              ...
              <button className="w-full py-2 px-4 text-left text-red-600 hover:bg-red-50 rounded-lg transition-all">
                🗑️ Delete Account
              </button>
            </div>
          </div>
